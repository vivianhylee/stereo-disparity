...
def build_segmentation(imageL):
    comp =segmentation.watershed(filters.sobel(imageL), markers=100, compactness=0.00001)
    return comp
...
def gen_disparity(imageL, imageR, ndisparities):
    pool = ThreadPool(2)
    disparity = pool.apply_async(build_disparity, args=(imageL, imageR, ndisparities))
    comp_map = pool.apply_async(build_segmentation, args=(imageL, ))
    pool.close()
    pool.join()
    result = fill_occlusion(disparity=disparity.get(), segmentation=comp_map.get())
    return result
...
def post_processing(image, ksize=3):
